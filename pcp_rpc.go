...
func GetPCPRPCClient(host string, port int, sandbox *gopcp.Sandbox, onClose goaio.OnCloseHandler) (*PCPConnectionHandler, error) {
	return GetPcpConnectionHandlerFromTcpConn(sandbox, func(onData goaio.BytesReadHandler) (goaio.ConnectionHandler, error) {
...
func GetPCPRPCPool(getAddress GetAddress, sandbox *gopcp.Sandbox, poolSize int, duration time.Duration, retryDuration time.Duration) *gopool.Pool {
	getNewItem := func(onItemBoken gopool.OnItemBorken) (*gopool.Item, error) {
		if host, port, err := getAddress(); err != nil {
			return nil, err
		} else {
			if pcpConnectionHandler, err := GetPcpConnectionHandlerFromTcpConn(sandbox, func(onData goaio.BytesReadHandler) (goaio.ConnectionHandler, error) {
